...
class TransactionAnalyzer
{
    public function analyze(string $text): array
    {
        ...
        // ... (Tu lógica de clasificación de TIPO se mantiene igual) ...
        if (preg_match('/^(retiro|disp\.? efect|cajero)/i', $textLower)) { $type = 'RETIRO'; } 
        elseif (preg_match('/^(deposito|abono|transf\.*? a favor)/i', $textLower)) { if (!str_contains($textLower, 'plin')) $type = 'DEPOSITO'; } 
        elseif (preg_match('/^(mant\.|comis\.|itf|estado de cta|seg\.|membresia)/i', $textLower)) { $type = 'MANTENIMIENTO'; }
        elseif (preg_match('/^(bcp\s*-|tran\.ctas|transf\.)/i', $textLower)) { $type = 'TRANSFERENCIA'; }
        elseif (preg_match('/(izi\*|niubiz|vendemas)/i', $textLower)) { $type = 'POS_NIUBIZ'; }
        elseif (preg_match('/(culqi)/i', $textLower)) { $type = 'POS_CULQI'; }
        elseif (preg_match('/(plin)/i', $textLower)) { $type = 'PLIN'; }
        elseif (preg_match('/(yape|yq-)/i', $textLower)) { $type = 'YAPE'; }
