...
return new class extends Migration
{
    ...
    {
        // 1. Delete Pareto assignments for income categories
        DB::statement("
            DELETE FROM category_pareto_assignments
            WHERE category_id IN (
                SELECT id FROM categories WHERE type = 'income'
            )
        ");

        // 2. Update Pareto function to exclude income categories and include user requested ordering
        $sql = <<<SQL
            ...
            BEGIN
                ...
                -- Calculate total budget for all classifications to get relative weight (Excluding Income)
...
SQL;
    ...
    {
        // For reverse, we would need to know what the previous state was.
        // Usually, these function migrations are overwritten by the next version.
    }
};
