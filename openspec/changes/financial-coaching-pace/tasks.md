...
- [ ] 4.5b RED `tests/Unit/Coaching/BlindnessDetectorTest.php`: a category with spend and `monthly_budget = 0` is reported as unwatchable; a category with `monthly_budget = 0` and **no** spend is not reported at all; a budgeted category never appears; income and savings are excluded; the report is one observation per user per month keyed `blindness`, never one per category.
