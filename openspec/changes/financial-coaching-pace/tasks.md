...
- [ ] 4.5d RED `tests/Feature/Coaching/CoachingKillSwitchTest.php`: with `coaching.enabled = false`, `speak()` returns `false`, claims nothing and sends nothing — asserted at the service, not merely that the config key exists. **It is the first line `speak()` executes.** The proposal's entire no-deploy rollback rests on this switch; slice 4a added the key with no consumer, so without this task the switch is decoration.
